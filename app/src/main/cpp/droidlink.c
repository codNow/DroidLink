...
theInputService = (*env)->NewGlobalRef(env, (*env)->FindClass(env, "com/codenow/droidlink/view/server/InputService"));
theMainService = (*env)->NewGlobalRef(env, (*env)->FindClass(env, "com/codenow/droidlink/view/server/MainService"));
...
JNIEXPORT jboolean JNICALL Java_com_codenow_droidlink_view_server_MainService_vncStopServer(__unused JNIEnv *env, __unused jobject thiz) {
...
Java_com_codenow_droidlink_view_server_MainService_vncStartServer(JNIEnv *env, jobject thiz,
                                                                  jint width, jint height,
                                                                  jint port, jstring desktop_name,
                                                                  jstring password) {
    int argc = 0;
    ...
    if(!theScreen->frameBuffer) {
        ...
        Java_com_codenow_droidlink_view_server_MainService_vncStopServer(env, thiz);
        return JNI_FALSE;
    }
    ...
    if(desktop_name) { // string arg to GetStringUTFChars() must not be NULL
        const char *cDesktopName = (*env)->GetStringUTFChars(env, desktop_name, NULL);
        if(!cDesktopName) {
        ...
        (*env)->ReleaseStringUTFChars(env, desktop_name, cDesktopName);
    } else
...
JNIEXPORT jboolean JNICALL
Java_com_codenow_droidlink_view_server_MainService_vncConnectReverse(JNIEnv *env, jobject thiz,
...
JNIEXPORT jboolean JNICALL
Java_com_codenow_droidlink_view_server_MainService_vncConnectRepeater(JNIEnv *env, jobject thiz,
                                                                      jstring host, jint port,
                                                                      jstring repeater_identifier) {
    if(!theScreen || !theScreen->frameBuffer)
        return JNI_FALSE;

    if(host && repeater_identifier) { // string arg to GetStringUTFChars() must not be NULL
        char *cHost = (char*)(*env)->GetStringUTFChars(env, host, NULL);
        if(!cHost) {
            __android_log_print(ANDROID_LOG_ERROR, TAG, "vncConnectRepeater: failed getting desktop name from JNI");
            return JNI_FALSE;
        }
        char *cRepeaterIdentifier = (char*)(*env)->GetStringUTFChars(env, repeater_identifier, NULL);
        ...
        (*env)->ReleaseStringUTFChars(env, host, cHost);
        (*env)->ReleaseStringUTFChars(env, repeater_identifier, cRepeaterIdentifier);
        return cl != NULL;
    }
    return JNI_FALSE;
}
...
JNIEXPORT jboolean JNICALL
Java_com_codenow_droidlink_view_server_MainService_vncNewFramebuffer(JNIEnv *env, jobject thiz,
                                                                     jint width, jint height) {
...
JNIEXPORT jboolean JNICALL
Java_com_codenow_droidlink_view_server_MainService_vncUpdateFramebuffer(JNIEnv *env, jobject thiz,
                                                                        jobject buf) {
...
JNIEXPORT jint JNICALL Java_com_codenow_droidlink_view_server_MainService_vncGetFramebufferWidth(__unused JNIEnv *env, jobject __unused thiz)
{
if(!theScreen || !theScreen->frameBuffer)
return -1;
...
Java_com_codenow_droidlink_view_server_MainService_vncGetFramebufferHeight(JNIEnv *env,
                                                                           jobject thiz) {
    if(!theScreen || !theScreen->frameBuffer)
        return -1;
...
JNIEXPORT jboolean JNICALL
Java_com_codenow_droidlink_view_server_MainService_vncIsActive(JNIEnv *env, jobject thiz) {
