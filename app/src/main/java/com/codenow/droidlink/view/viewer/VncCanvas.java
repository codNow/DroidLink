...
import com.codenow.droidlink.view.antlersoft.android.bc.BCFactory;
...
public class VncCanvas extends androidx.appcompat.widget.AppCompatImageView {
	...
	void initializeVncCanvas(final VncSettings settings, final Runnable setModes) {
		...
		customDialog.requestWindowFeature(Window.FEATURE_NO_TITLE); // Remove the title bar
		...
		dialogMessage.setText("Connecting...");
		...
		Thread t = new Thread() {
			public void run() {
				try {
					...
					// Update the text message within the custom dialog
					handler.post(new Runnable() {
						public void run() {
							dialogMessage.setText("Connecting Please wait...");
						}
					});

					// Continue with the rest of your background processing
					processNormalProtocol(getContext(), customDialog, setModes);
				} catch (Throwable e) {
					if (maintainConnection) {
						Log.e(TAG, e.toString());
						e.printStackTrace();
						// Ensure we dismiss the custom dialog
					...
					// Dismiss the custom dialog once the background task is complete
					customDialog.dismiss();
				}
			}
		};
	...
	void doProtocolInitialisation(int dx, int dy) throws IOException {
		...
		Log.i(TAG, "Device name is " + rfb.desktopName);
		Log.i(TAG, "Device size is " + rfb.framebufferWidth + " x " + rfb.framebufferHeight);
	...
	public void processNormalProtocol(final Context context, final Dialog dialog, final Runnable setModes) throws Exception {
			...
			while (maintainConnection) {
				...
				case RfbProto.FramebufferUpdate:
					...
					for (int i = 0; i < rfb.updateNRects; i++) {
						...
						if (rfb.updateRectEncoding == RfbProto.EncodingXCursor || rfb.updateRectEncoding
						...
						case RfbProto.EncodingZlib:
							...
							break;
						default:
							Log.e(TAG, "Unknown RFB rectangle encoding "
									+ rfb.updateRectEncoding + " (0x" + Integer.toHexString(rfb.updateRectEncoding) + ")");
						}
